(() => {
  ...
  function agreement() { const p = policy(), slot = $("policy-agreement"); if (!p) return false; const label = document.createElement("label"), check = document.createElement("input"), span = document.createElement("span"); const documents = [["Event Terms", "./event-terms.html"], ["Privacy Notice", "./event-privacy.html"], ["Media Release", "./media-release.html"]]; label.className = "check"; check.type = "checkbox"; check.name = "combined_agreement"; check.required = true; span.append("I agree to the "); for (const [name, href] of documents) { const a = document.createElement("a"); a.href = href; a.target = "_blank"; a.rel = "noopener noreferrer"; a.textContent = name; span.append(a, document.createTextNode(name === "Media Release" ? ", and to event texts. Message and data rates may apply; reply STOP to stop." : ", ")); } label.append(check, span); slot.replaceChildren(label); return true; }
  ...
  async function load() {
    ...
    if (!base() || !policy()) { $("context-lede").textContent = "Registration is not available right now."; status("Registration is temporarily unavailable. Please write to ssuite@salute.community and we will help you complete it.", "error"); return; }
    let result;
  ...
  async function submit(event) {
    ...
    try {
      ...
      const response = await api({ attendee, how_heard: howHeard || undefined, combined_agreement: agreed.checked === true });
