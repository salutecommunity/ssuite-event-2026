...
function renderTables(host, payload) {
  ...
  for (const table of payload.data || []) {
    ...
    if (lead && lead.registration_status === "complete") {
      ...
      reveal.addEventListener("click", async () => {
        ...
        try {
          ...
          linkBox.replaceChildren(element("p", "fineprint", `Private management access for ${text(result.recipient)}. Share only with the named table host. Nothing was emailed.`), input, copy); linkBox.hidden = false; input.focus(); input.select();
    ...
    for (const seat of table.table_seats || []) {
      ...
      const guestName = attendee ? `${text(attendee.first_name)} ${text(attendee.last_name)}${attendee.is_table_lead ? " · Host" : ""}` : seat.locked ? "Locked" : "Open";
...
function setLeadForm(table) {
  const seated = (table.table_seats || []).filter((seat) => seat.attendee).map((seat) => ({ value: seat.attendee.id, label: `${text(seat.attendee.first_name)} ${text(seat.attendee.last_name)} · seat ${seat.seat_number}` })); const form = element("form", "attendee-edit"); form.append(selectField("Table host", "attendee_id", [{ value: "", label: "Clear table host" }, ...seated], table.lead_attendee_id || "", false)); operationActions(form, "Save table host", async (data) => { const value = data.get("attendee_id"); await call({ action: "admin_table_lead_set", table_id: table.id, attendee_id: value || null }); status("Table host updated. Nothing was sent.", "success"); }); openOperation("Set or clear table host", "The selected host must occupy a seat. The operation is blocked while an active table-management credential exists; no credential is created, changed, revoked, or sent.", () => form);
}
